#!/usr/bin/env python3
"""
film_emulation_impl.py — Grain-free version

This file is the cleaned, deduplicated, and grain-free variant of the
film emulation training + inference script you provided.

Changes:
- Removed all film grain code (class, usage, losses, checkpoints).
- Merged training loops into a single stable training function.
- Kept LPIPS optional handling.
- Kept PerceptualLoss, identity/cycle/adversarial losses, schedulers, EMA, etc.
- The 'apply' mode now performs only the per-pixel affine color/tone mapping.

Usage examples:
  Train:
    python film_emulation_impl.py --mode train_unpaired --data ./dataset_root
  Apply:
    python film_emulation_impl.py --mode apply --model best_model_G.pth --src ./digital --out ./out
  Eval LPIPS:
    python film_emulation_impl.py --mode eval_lpips --model best_model_G.pth --model_f best_model_F.pth --data ./dataset_root --out ./out
"""
...
import torch.nn.functional as Ff  # functional API alias to avoid naming collisions with generator F
...
class PerPixelAffineNet(nn.Module):
    ...
    def __init__(self, base_ch=32):
        ...
        with torch.no_grad():
            self.out_conv.weight.normal_(0, 0.01)  # Small random init
            identity_bias = torch.tensor(
                [1, 0, 0, 0, 1, 0, 0, 0, 1, 0, 0, 0], dtype=torch.float32
            )
            identity_bias += torch.randn(12) * 0.05  # Add noise
...
class PerceptualLoss(nn.Module):
    ...
    def __init__(self, device="cpu"):
        super().__init__()
        # torchvision older/newer API differences: pretrained=True is common
...
def train_unpaired(
    ...
    # Optimizers with weight decay (no GrainNet parameters)
    ...
    for epoch in range(epochs):
        ...
        for it in range(num_batches):
            ...
            # =============== Generators Forward (produce fake images) ===============
            tmap_g = G(real_dig)
            fake_film = torch.clamp(apply_perpixel_affine(tmap_g, real_dig), 0.0, 1.0)
            ...
            loss_id = 0.5 * (loss_id_g + loss_id_f)
            ...
            if (it) % 10 == 0:
                print(
                    ...
                    f"D: {loss_d.item():.4f} | G: {loss_g_total.item():.4f} | "
        ...
        history["d_loss"].append(epoch_d_loss / max(1, num_batches))
        ...
        if (epoch) % val_interval == 0:
            ...
            with torch.no_grad():
                ...
                for val_dig, val_film in val_pairs:
                    ...
                    # Check transform map statistics
                    print(
                        f"Transform map - mean: {tmap_g.mean():.4f}, std: {tmap_g.std():.4f}"
                    )
                    print(f"Matrix part mean: {tmap_g[:,:9].mean():.4f}")
                    print(f"Bias part mean: {tmap_g[:,9:].mean():.4f}")
            ...
            if lpips_metric is not None and len(lpips_dig2film_scores) > 0:
                ...
                history["val_lpips_dig2film"].append(float(avg_lpips_d2f))
                history["val_lpips_film2dig"].append(float(avg_lpips_f2d))
                history["val_lpips_cycle_dig"].append(float(avg_lpips_cycle_d))
                history["val_lpips_cycle_film"].append(float(avg_lpips_cycle_f))
...
def apply_trained_generator(
    ...
    """Apply trained generator (color-only) to all images in src_folder.

    direction is kept for API compatibility; 'G' is digital->film.
    """
    if torch.backends.mps.is_available() and device == "mps":
        device = torch.device("mps")
    elif torch.cuda.is_available() and device == "cuda":
    ...
    for img_path in sorted(
        ...
        with torch.no_grad():
            ...
            out = out_base  # Grain removed — only color/tone transform
...
def evaluate_lpips_on_folder(
    ...
    """
    LPIPS evaluation on paired samples between digital_folder and film_folder.
    Grain is removed from evaluation — only color mapping is considered.
    """
    if not LPIPS_AVAILABLE:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Film Emulation (grain-free) with CycleGAN-style training and LPIPS evaluation"
    )
    parser.add_argument(
        "--data",
        type=str,
