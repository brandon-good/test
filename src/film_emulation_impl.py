...
def train_unpaired(
    ...
    # Initialize models
    # Single initialization — removed accidental duplicates
    ...
    # Optimizers
    # Generator optimizer includes GrainNet so grain weights are updated with the generators
    ...
    for epoch in range(epochs):
        ...
        for it in range(min(len(digital_loader), len(film_loader))):
            ...
            # Add grain as a residual (small magnitude) and clamp to valid range
            ...
            # ---- Smoothness regularization for affine maps ----
            def smoothness_loss(tmap):
                return torch.nn.functional.l1_loss(
                    tmap[:, :, :-1, :], tmap[:, :, 1:, :]
                ) + torch.nn.functional.l1_loss(tmap[:, :, :, :-1], tmap[:, :, :, 1:])

            # ---- High-pass filter to constrain grain to HF only ----
            hp_kernel = torch.tensor(
                [[-1, -1, -1], [-1, 8, -1], [-1, -1, -1]],
                dtype=torch.float32,
                device=device,
            ).view(1, 1, 3, 3)
            if "grain_map" in locals():
                grain_hp = torch.nn.functional.conv2d(grain_map, hp_kernel, padding=1)
                grain_lp = grain_map - grain_hp
                loss_grain_reg = torch.mean(grain_lp**2)
            else:
                loss_grain_reg = torch.tensor(0.0, device=device)
            ...
            grain_weight = 0.02  # reduced grain strength

            # ---- FINAL GENERATOR LOSS (fixed) ----
            loss_g = (
                loss_g_adv_film
                + loss_f_adv_dig
